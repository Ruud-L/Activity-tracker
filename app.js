...
function updateSeoMeta() {
  const metaTitle = t("app.metaTitle") || "Activity Tracker - Download Hub";
  const metaDescription =
    t("app.metaDescription") ||
    `${t("app.description1") || "A fully local app for tracking activity and measuring productivity."} ${t("app.description2") || ""}`.trim();

  document.title = metaTitle;

  const canonical = `${window.location.origin}${window.location.pathname}`;
  const imageUrl = new URL("./Activity_tracker_logo.png", window.location.href).href;

  setMetaContent("meta-description", metaDescription);
  setMetaContent("og-title", metaTitle);
  setMetaContent("og-description", metaDescription);
  setMetaContent("og-url", canonical);
  setMetaContent("og-image", imageUrl);
  setMetaContent("twitter-title", metaTitle);
  setMetaContent("twitter-description", metaDescription);
...
function registerRevealAnimations() {
  ...
  // Hysteresis + minimum toggle interval for stable, smooth feel.
  // Show a bit earlier, hide only when almost out of view.
  const SHOW_RATIO = 0.14;
  const HIDE_RATIO = 0.02;
  const MIN_TOGGLE_MS = 180;
  const visibleState = new Map();
  const lastToggleAt = new Map();
  panels.forEach((panel) => visibleState.set(panel, false));

  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      const panel = entry.target;
      const wasVisible = visibleState.get(panel) === true;
      const ratio = entry.intersectionRatio;
      const now = performance.now();
      const last = lastToggleAt.get(panel) || 0;

      if (now - last < MIN_TOGGLE_MS) {
        return;
      }

      if (!wasVisible && ratio >= SHOW_RATIO) {
        panel.classList.add("is-visible");
        visibleState.set(panel, true);
        lastToggleAt.set(panel, now);
      } else if (wasVisible && ratio <= HIDE_RATIO) {
        panel.classList.remove("is-visible");
        visibleState.set(panel, false);
        lastToggleAt.set(panel, now);
      }
    });
  }, {
    threshold: [0, HIDE_RATIO, SHOW_RATIO, 0.35, 1],
